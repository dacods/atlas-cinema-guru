"use client";

import { useSession } from "next-auth/react";
import { useEffect, useState } from "react";
import MovieCard from "@/components/MovieCard";
import Pagination from "@/components/Pagination";
import Header from "@/components/Header";
import Sidebar from "@/components/Sidebar";

export default function WatchLaterPage() {
  const { data: session } = useSession();
  const [page, setPage] = useState(1);
  const [watchLaters, setWatchLaters] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const loadWatchLaters = async () => {
      setLoading(true);
      try {
        const res = await fetch(`/api/watch-later?page=${page}`, {
          credentials: "include",
        });
        const data = await res.json();
        setWatchLaters(data.watchLater || []);
      } catch (err) {
        console.error("Error fetching watch later list:", err);
        setWatchLaters([]);
      } finally {
        setLoading(false);
      }
    };

    if (session?.user?.email) {
      loadWatchLaters();
    }
  }, [page, session?.user?.email]);

  return (
    <>
      <Header />
      <div className="flex group h-screen">
        <Sidebar userEmail={session?.user?.email ?? ""} />
        <main className="transition-all duration-300 ml-16 group-hover:ml-64 flex-1 p-6">
          <h1 className="text-3xl font-bold text-center mb-6">Watch Later</h1>
          {loading ? (
            <p className="text-center">Loading...</p>
          ) : watchLaters.length === 0 ? (
            <p className="text-center">You have no movies in your Watch Later list.</p>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              {watchLaters.map((movie) => (
                <MovieCard key={movie.id} movie={movie} />
              ))}
            </div>
          )}
          <Pagination
            currentPage={page}
            onPrev={() => setPage((p) => Math.max(1, p - 1))}
            onNext={() => setPage((p) => p + 1)}
            disablePrev={page === 1}
            disableNext={watchLaters.length < 6}
          />
        </main>
      </div>
    </>
  );
}
